...
SimulationTime = 150;     % Change this if you want to change the simulation duration
counter = 0;
...
for i = 1:size(q_ref,1)
    ...
    T_all(:,i) = T(end,:)';
    if(i>1)
        [t,x] = ode45(@(t,x) odefcn(t,x,tau_ref,coeff), sample_time*[i-1 i], [q_ref(i,:)';q_dot_ref(i,:)']);
