...
R = 0.18; % Wheel dist
L = 0.18; % Distance between wheels
r = 0.0325; % Radius of each wheel
m = 1.5;   % Robot mass
...
% turn it into m/s
left_wheel_vel = r*2*pi*(left_speed/60);
right_wheel_vel = r*2*pi*(right_speed/60);
...
%% Initial state
q_init = [0, 0, 0];  % Initial state [x_c, y_c, theta]
q_dot_init = [0, 0, 0];          % Initial linear velocity
omega_init = 0;      % Initial angular velocity

%% Parameters of the Circular Trajectory
a = 3;       % Circle Center in meters (X Axis)
c = 5;       % Circle Center in meters (Y Axis)
b = 2;       % Radius in meters
omega_r = (right_wheel_vel - left_wheel_vel)/L; % Angular velocity for theta reference
...
for i = 1:size(q_ref,1)
    ...
    T = Phi * w;
    if(i>1)
        [t,x] = ode45(@(t,x) odefcn(t,x,T), sample_time*[i-1 i], [q_ref(i,:)';q_dot_ref(i,:)']);
